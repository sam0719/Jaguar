...
float DOMAIN_LOWER = -32.769;

bool epin = 1;// 控制輸出epin
...
void epin_write(string address, string Info, float best_location, float now_location) {
    if (JA_count == 1) {
        ...
        f << "Dimension : 1\nFormula : 20+exp(1)-20*exp((0-0.2*(sum(Xi^2)))/1^0.5)-exp(sum(cos(2*PI*Xi))/1)\nRange : " << DOMAIN_LOWER << " ~ " << DOMAIN_UPPER << "\nPosition :" << endl;
...
double cal_fitness(float x, string Info) {
    double r = 0.0,sum1 = 0.0,sum2 = 0.0;
    ...
    sum1 = sum1 / (double)DIMENSION;
    sum2 = sum2 / (double)DIMENSION;
    sum1 = pow(sum1, 0.5);
    sum1 = -20.0 * exp(0.0 - 0.2 * sum1) + 20.0;
    sum2 = -exp(sum2) + exp(1.0);
    r = sum1 + sum2;
